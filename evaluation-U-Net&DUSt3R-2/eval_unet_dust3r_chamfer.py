#!/usr/bin/env python3
"""Evaluate DUSt3R vs KD student (StudentRestorationFrontEnd) + DUSt3R on clean/motion/defocus.

Six scenarios, Chamfer vs CO3D GT point cloud; per-scenario mean over all evaluated sequences.
Default categories: bottle, cup, donut, teddybear, couch, toytrain (same as eval v1).
"""
...
def main():
    ap = argparse.ArgumentParser("Evaluate DUSt3R and KD student + DUSt3R Chamfer (6 scenarios)")
    ap.add_argument("--co3d_processed", required=True)
    ap.add_argument("--co3d_raw", default=None)
    ap.add_argument(
        "--split",
        default="test",
        help="Split suffix for selected_seqs_<split>.json (use `test` for bottle/cup/… on co3d_processed).",
    )
    ...
    if not scenes:
        avail = sorted(k for k, v in split_data.items() if isinstance(v, dict))
        raise RuntimeError(
            f"No scenes for categories {list(args.categories)} in split {args.split!r}. "
            f"Categories present in this JSON: {avail}. "
            f"Pick a split + --co3d_processed root that include your --categories (e.g. co3d_processed + test)."
        )
    ...
    print(f"[startup] student_ckpt={args.student_ckpt}", flush=True)
    ...
    ck = _torch_load_ckpt(args.student_ckpt)
    ...
    tmp_root = tempfile.mkdtemp(prefix="eval_unet_dust3r2_")
    try:
