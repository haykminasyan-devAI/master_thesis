#!/usr/bin/env python3
"""Fine-tune DeblurDiNAT + DUSt3R on CO3D with blurred images (DDP-ready)."""
...
from finetune_blur.datasets.co3d_blur import Co3dBlur


def get_args_parser():
    p = argparse.ArgumentParser('DeblurDiNAT + DUSt3R fine-tuning')
    p.add_argument('--co3d_root', required=True)
    p.add_argument('--blur_root', required=True)
    p.add_argument('--blur_sigmas', type=int, nargs='+', default=[10],
                   help='one or more blur sigmas to train on jointly (e.g. 5 10 20)')
    ...
    p.add_argument('--deblurdinat_weights', default=None,
                   help='path to DeblurDiNATL.pth (GoPro pretrained)')
    p.add_argument('--output_dir', default='./output_deblurdinat')
    p.add_argument('--freeze', default='deblurdinat_only',
    ...
    p.add_argument('--eta_min', type=float, default=1e-7, help='cosine LR floor')
    p.add_argument('--grad_clip', type=float, default=1.0,
                   help='max grad norm; 0 disables')
    p.add_argument('--resolution', type=int, default=512)
    p.add_argument('--amp', type=int, default=1, choices=[0, 1],
                   help='mixed precision (recommended: 1 for memory)')
    p.add_argument('--grad_checkpoint', type=int, default=1, choices=[0, 1],
                   help='checkpoint DUSt3R blocks (recommended: 1 for memory)')
    p.add_argument('--deblur_checkpoint', type=int, default=1, choices=[0, 1],
                   help='checkpoint full DeblurDiNAT forward (NATTEN); strongly recommended')
    p.add_argument('--symmetrize_batch', type=int, default=0, choices=[0, 1],
                   help='1=symmetric pair loss like DUSt3R (2× memory); 0 saves VRAM')
...
def main():
    ...
    sigmas = sorted(set(args.blur_sigmas))
    per_sigma = [
        Co3dBlur(
            blur_sigma=sigma,
            blur_root=args.blur_root,
            split='train',
            ROOT=args.co3d_root,
    ...
    print(f'Dataset: {len(dataset)} pairs (blur_sigmas={sigmas})')
    ...
    if args.distributed:
        # gradient_as_bucket_view=False avoids "Grad strides do not match bucket view" warnings
        # on some params (e.g. small 1×1×3 tensors) with negligible perf cost.
        # find_unused_parameters=False: faster; set True if backward errors about unused grads.
    ...
    if wu <= 0:
        ...
        scheduler = LinearLR(
            optimizer, start_factor=1e-6, end_factor=1.0, total_iters=args.epochs)
    else:
        warmup = LinearLR(
            optimizer, start_factor=1e-6, end_factor=1.0, total_iters=wu)
        cosine = CosineAnnealingLR(
            optimizer, T_max=max(1, args.epochs - wu), eta_min=args.eta_min)
