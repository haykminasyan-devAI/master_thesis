#!/usr/bin/env bash
# ASDS (dgx): Uformer + DUSt3R motion finetune — same code as YSU seq8 script, local defaults only.
#
# Prereqs under PROJECT_DIR (defaults):
#   data/co3d_processed_10cat8seq_fixed/
#   data/co3d_selected_jsons/*_selected_8.json  (10 files)
#   outputs/degraded_frames_motion_10cat/<cat>/<seq>/<MOTION_TAG>/frame*.jpg
#   checkpoints/DUSt3R_ViTLarge_BaseDecoder_512_dpt.pth
#   checkpoints/uformer/Uformer_B.pth
#
# Usage (no long --export line; override vars only if needed):
#   cd ~/project_Hayk_Minasyan
#   sbatch finetune_motion_blur/train_uformer_front_dust3r_motion_asds.sh
#
# Optional: sbatch --export=ALL,EPOCHS=10,OUT_DIR=/path finetune_motion_blur/train_uformer_front_dust3r_motion_asds.sh

#SBATCH -J uformer_dust3r_motion_asds
#SBATCH -p a100
#SBATCH --gres=gpu:a100:1
#SBATCH --cpus-per-task=16
#SBATCH --mem=128G
#SBATCH -t 96:00:00
#SBATCH -o logs/uformer_dust3r_motion_asds_%j.log
#SBATCH -e logs/uformer_dust3r_motion_asds_%j.err
...
OUT_DIR="${OUT_DIR:-${PROJECT_DIR}/finetune_motion_blur_runs/uformer_dust3r_seq8_motion_asds}"
...
WANDB_RUN_NAME="${WANDB_RUN_NAME:-uformer_dust3r_motion_asds_lora_bs2_lr2e5}"
