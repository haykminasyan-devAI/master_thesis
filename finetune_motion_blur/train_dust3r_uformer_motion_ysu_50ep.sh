#!/usr/bin/env bash
#SBATCH --job-name=dust3r_uformer_motion_50ep
...
#SBATCH --output=/home/hminasyan/project_Hayk_Minasyan/logs/dust3r_uformer_motion_50ep_%j.log
#SBATCH --error=/home/hminasyan/project_Hayk_Minasyan/logs/dust3r_uformer_motion_50ep_%j.err
...
: "${UFORMER_WEIGHTS:=/mnt/weka/hminasyan/checkpoints/uformer/Uformer_B.pth}"
: "${OUTPUT_DIR:=/mnt/weka/hminasyan/finetune_motion_blur_runs/uformer_dust3r_ysu_224_temporal_avg_w11_50ep}"
: "${FREEZE:=uformer_only}"
...
: "${WANDB_RUN_NAME:=uformer_dust3r_motion_temporal_avg_w11_ysu_50ep}"
...
"${VENV_PYTHON}" -m torch.distributed.run \
  ...
  "${PROJECT_DIR}/finetune_motion_blur/train_uformer_front_dust3r.py" \
  --co3d_root "${CO3D_ROOT}" \
