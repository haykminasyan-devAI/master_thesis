"""
Line plot: Number of frames vs Chamfer Distance for grid masking
at 3 mask ratios (10%, 25%, 50%), averaged over 6 CO3D categories.

Expected directory structure:
    outputs/dust3r/grid_mask_exp/
        mask_10pct/
...
Usage:
    python scripts/grid_mask_exp/plot_grid_mask_exp.py \\
        --results_root outputs/dust3r/grid_mask_exp \\
        --n_min 2 --n_max 20
"""
...
MASK_TAGS   = ["mask_10pct", "mask_20pct", "mask_30pct", "mask_40pct", "mask_50pct"]
MASK_LABELS = ["Grid mask 10%", "Grid mask 20%", "Grid mask 30%", "Grid mask 40%", "Grid mask 50%"]
COLORS      = ["#2E7D32", "#66BB6A", "#1565C0", "#E65100", "#B71C1C"]
MARKERS     = ["o", "s", "^", "D", "v"]
LINESTYLES  = ["-", "--", "-.", ":", "--"]
...
def plot(data, n_min, n_max, out_path):
    ...
    for i, tag in enumerate(MASK_TAGS):
        ...
        results[tag] = (ns, means, stds_v, n_seq)
        ...
        # annotate last point with offset to avoid overlap
        offsets = [12, 4, -4, -12, -20]
    ...
    # ── Y-axis: zoom into actual data range ──────────────────────────────────
    if all_means_flat:
...
def main():
    ...
    parser.add_argument("--results_root", default="outputs/dust3r/grid_mask_exp")
    ...
    out_path = os.path.join(out_dir, "grid_mask_compare.png")
