...
Includes all fixes:
  - Fix 1: Non-squared Chamfer (in run_dust3r_inference.py)
  - Fix 2: Run Umeyama on RAW cameras (no pre-normalization)
  - Fix 3: Subsampling before metrics computation
  - Fix 4: Apply FULL Umeyama transform (scale + R + t)
...
def main():
    ...
    if not args.skip_sfm:
        ...
        # Step 2: Sequential feature matching (GPU, loop detection disabled)
        run_cmd([
    ...
    # ── Load GT point cloud (needed for normalization) ────────────────────────
    gt_ply = os.path.join(args.sequence_dir, "pointcloud.ply")
    if not os.path.isfile(gt_ply):
        print(f"ERROR: GT point cloud not found at {gt_ply}")
        with open(metrics_txt, "w") as f:
            f.write(f"n_frames:  {args.n_frames}\nstatus:    no_gt_ply\n")
        return
    ...
    if matched < 3:
        ...
        # ── FIX 2: Run Umeyama on RAW cameras (NO pre-normalization) ──────────
        colmap_std_raw = np.std(pred_pts_raw, axis=0).mean()
        gt_std = np.std(gt_pts, axis=0).mean()
        
        print(f"\n[STD NORMALIZATION]")
        print(f"  COLMAP STD (raw): {colmap_std_raw:.4f}")
        print(f"  GT STD: {gt_std:.4f}")
        
        # Run Umeyama on RAW camera centers (gets full similarity transform)
        scale_umeyama, R_align, t_align = umeyama(
            np.array(src_pts), np.array(dst_pts)  # ← RAW, not normalized!
        )
        print(f"  Umeyama scale (raw cameras): {scale_umeyama:.4f}")
        
        # Apply FULL transform to RAW points (including Umeyama scale!)
        pred_pts = apply_similarity(pred_pts_raw, scale_umeyama, R_align, t_align)
        print(f"  Applied transform: scale={scale_umeyama:.4f}, rotation + translation")
        
        # Verify alignment
        aligned_std = np.std(pred_pts, axis=0).mean()
        print(f"  Aligned COLMAP STD: {aligned_std:.4f} (GT STD: {gt_std:.4f})")
    ...
    # ── FIX 3: Subsample for fair, fast metrics ───────────────────────────────
    MAX_POINTS = 100000
    if len(pred_pts) > MAX_POINTS:
        print(f"\n[SUBSAMPLE] Prediction: {len(pred_pts):,} → {MAX_POINTS:,}")
